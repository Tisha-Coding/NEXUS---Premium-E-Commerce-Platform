...
const getUserCart = async (req, res) => {
  try {
    ...
    // Check if any products in cart have been deleted
    ...
    res.json({
      success: true,
      cartData,
      removedItems: deletedItems.length > 0 ? deletedItems : undefined,
      message: deletedItems.length > 0 ? `${deletedItems.length} item(s) were removed from your cart because they are no longer available.` : undefined,
    });
  } catch (error) {
    res.json({ success: false, message: error.message });
  }
